@extends('layouts.master')
@section('title', 'Ajout nouveau Rattrapage')
@section('content')
    <div class="wrapper">
        <!-- Content Wrapper. Contains page content -->
        <div class="content-wrapper">
            <!-- Content Header (Page header) -->
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-12">
                            <h1 class="text-center">Nouveau Rattrapage</h1>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->
            </section>

            <!-- Main content -->
            <section class="content">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card card-info">
                                <div class="card-header">
                                    <h3 class="card-title">Ajout d'un nouveau Rattrapage</h3>
                                </div>
                                <div class="card-body">
                                    <div class="mt-4">
                                        @if (\Session::has('success'))
                                            <div class="alert alert-success">
                                                <ul>
                                                    <li>{!! \Session::get('success') !!}</li>
                                                </ul>
                                            </div>
                                        @endif
                                        @if ($errors->any())
                                            <div class="alert alert-danger">
                                                <ul>
                                                    @foreach ($errors->all() as $error)
                                                        <li>{{ $error }}</li>
                                                    @endforeach
                                                </ul>
                                            </div>
                                        @endif
                                        <form action="{{ route('rattrapages.store') }}" method="post">
                                            {{ csrf_field() }}
                                            {{ method_field('post') }}

                                            <div class="row">
                                                <div class="form-group col-md-6">
                                                    <label class="form-control-label" for="classe_id">Classe:</label>
                                                    <div class="input-group">
                                                        <select name="classe_id"
                                                            class="form-control form-control-alternative {{ $errors->has('classe_id') ? 'is-invalid' : '' }}">
                                                            <option disabled selected>--- Choisissez une classe ---</option>
                                                            @foreach ($classes as $classe)
                                                                <option value="{{ $classe->id }}">{{ $classe->nom }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        @error('classe_id')
                                                            <span class="help-block invalid-feedback">{{ $message }}</span>
                                                        @enderror
                                                    </div>
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label class="form-control-label" for="matiere_id">UE:</label>
                                                    <div class="input-group">
                                                        <select name="matiere_id"
                                                            class="form-control form-control-alternative {{ $errors->has('matiere_id') ? 'is-invalid' : '' }}">
                                                            <option disabled selected>--- Choisissez une matière ---
                                                            </option>
                                                            @foreach ($matieres as $matiere)
                                                                <option value="{{ $matiere->id }}">{{ $matiere->nom }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        @error('matiere_id')
                                                            <span
                                                                class="help-block invalid-feedback">{{ $message }}</span>
                                                        @enderror
                                                    </div>
                                                </div>
                                            </div>
                                            {{-- <div class="row">
                                                <div class="form-group col-md-6">
                                                    <label class="form-control-label" for="matiere_id">Coefficient:</label>
                                                    <div class="input-group">
                                                        <select name="matiere_id"
                                                            class="form-control form-control-alternative {{ $errors->has('matiere_id') ? 'is-invalid' : '' }}">
                                                            <option disabled selected>--- Choisissez un coefficient ---
                                                            </option>
                                                            @foreach ($matieres as $matiere)
                                                                <option value="{{ $matiere->id }}">
                                                                    {{ $matiere->coefficient }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        @error('matiere_id')
                                                            <span
                                                                class="help-block invalid-feedback">{{ $message }}</span>
                                                        @enderror
                                                    </div>
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label class="form-control-label" for="matiere_id">Eléments
                                                        Constitutifs:</label>
                                                    <div class="input-group">
                                                        <select name="matiere_id"
                                                            class="form-control form-control-alternative {{ $errors->has('matiere_id') ? 'is-invalid' : '' }}">
                                                            <option disabled selected>--- Choisissez un élément constitutif
                                                                ---
                                                            </option>
                                                            @foreach ($matieres as $matiere)
                                                                <option value="{{ $matiere->id }}">
                                                                    {{ $matiere->coefficient }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        @error('matiere_id')
                                                            <span
                                                                class="help-block invalid-feedback">{{ $message }}</span>
                                                        @enderror
                                                    </div>
                                                </div>
                                            </div> --}}
                                            <div class="row">
                                                {{-- <div class="form-group col-md-6">
                                                    <label>Responsables:</label>
                                                    <div class="input-group">
                                                        <input id="date" type="text" placeholder="2022-10-15"
                                                            class="form-control @error('date') is-invalid @enderror"
                                                            name="date" value="{{ old('date') }}" required
                                                            autocomplete="date" autofocus>
                                                    </div>
                                                    @error('date')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                        </span>
                                                    @enderror
                                                </div> --}}
                                                <div class="form-group col-md-12">
                                                    <label>Date Examen:</label>
                                                    <div class="input-group">
                                                        <input id="date" type="text" placeholder="2022-10-15"
                                                            class="form-control @error('date') is-invalid @enderror"
                                                            name="date" value="{{ old('date') }}" required
                                                            autocomplete="date" autofocus>
                                                    </div>
                                                    @error('date')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                        </span>
                                                    @enderror
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="form-group col-md-6">
                                                    <label>Horaire:</label>
                                                    <div class="input-group">
                                                        <input id="heure" type="text" placeholder="07h:00 - 10h:00"
                                                            class="form-control @error('heure') is-invalid @enderror"
                                                            name="heure" value="{{ old('heure') }}" required
                                                            autocomplete="heure" autofocus>
                                                    </div>
                                                    @error('heure')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                        </span>
                                                    @enderror
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label>Effectif Etudiant:</label>
                                                    <div class="input-group">
                                                        <input id="effectif" type="text" placeholder="25"
                                                            class="form-control @error('date') is-invalid @enderror"
                                                            name="effectif" value="{{ old('effectif') }}" required
                                                            autocomplete="effectif" autofocus>
                                                    </div>
                                                    @error('effectif')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                        </span>
                                                    @enderror
                                                </div>
                                            </div>
                                            <button type="submit" class="btn btn-success">Enregistrer</button>
                                            <!-- /.form -->
                                        </form><br>
                                    </div>
                                    <!-- /.card-body -->
                                </div>
                                <!-- /.card -->
                            </div>
                            <!-- /.col (left) -->
                        </div>
                        <!-- /.row -->
                    </div>
                    <!-- /.container-fluid -->
            </section>
            <!-- /.content -->
        </div>
        <!-- /.content-wrapper -->

    </div>
@endsection
@section('script')
    <script src="{{ asset('assets/dist/js/adminlte.min.js') }}"></script>
@endsection
